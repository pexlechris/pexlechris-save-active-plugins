...
add_action(
	...
	function ( $wp_admin_bar ) {
		...
		if ( ! current_user_can( PEXLECHRIS_SAVE_PLUGINS_CAP ) ) {
			return;
		}

		$saved_plugins = get_option( 'pexlechris_saved_plugins', array() );

		$query              = $_GET;
		$query['pexlepage'] = 'save_plugins';
		$query['_wpnonce']  = wp_create_nonce( 'save_plugins_nonce' );
		$query_result       = http_build_query( $query );
		if ( '' != $query_result ) {
			$query_result = '?' . $query_result;
		}
		$url = wp_parse_url( $_SERVER['REQUEST_URI'], PHP_URL_PATH ) . $query_result;
...
add_action(
	...
	function () {
		...
		if ( ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( $_GET['_wpnonce'], 'save_plugins_nonce' ) ) {
			wp_die( 'Invalid nonce' );
		}

		$pexlepage = $_GET['pexlepage'] ?? '';
		if ( $pexlepage != 'save_plugins' ) {
			return;
		}
...
add_action(
	...
	function () {
		...
		foreach ( $saved_plugins as $saved_plugin ) {
			...
			add_filter(
				"plugin_action_links_$saved_plugin",
				function ( $links ) {
...
add_filter(
	...
	function ( $views ) {
		...
		$fav_filter_link = add_query_arg(
			array( 'plugin_status' => 'fav_plugins' ),
			admin_url( 'plugins.php' )
		);

		$fav_plugins = get_option( 'pexlechris_saved_plugins', array() );
...
add_action(
	...
	function () {
		...
		$wp_list_table->items = array_filter(
			$wp_list_table->items,
			function ( $plugin_data, $plugin_file ) use ( $fav_plugins ) {
...
add_filter(
	...
	function ( $url ) {
		...
		$url = add_query_arg( array( 'plugin_status' => 'fav_plugins' ), $url );
